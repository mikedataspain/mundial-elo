...
def _fetch_dia_espn(fecha_yyyymmdd: str) -> list[tuple[str, str, int, int]]:
    """
    Obtiene los partidos terminados de un día desde la ESPN public API.
    Devuelve lista de (eq1_es, eq2_es, pts_eq1, pts_eq2).
    """
    url = (
    ...
    for event in data.get("events", []):
        ...
        partidos.append((eq1, eq2, pts1, pts2))
...
def obtener_resultados_jugados(
    ...
    Retorna:
        {frozenset({eq1, eq2}): {eq1: pts_eq1, eq2: pts_eq2}}
    ...
    for fecha_str in fechas_pasadas:
        ...
        for eq1, eq2, pts1, pts2 in dia:
            key = frozenset({eq1, eq2})
            resultados[key] = {eq1: pts1, eq2: pts2}
